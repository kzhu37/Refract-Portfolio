...
> [!IMPORTANT]
> **Refract is an experimental research and engineering prototype.** It is not a medical device, clinical diagnostic tool, or replacement for glasses, contact lenses, professional eye examinations, or other vision care. The guided workflow produces heuristic estimates only.
...
| **Verification** | Nine focused numerical and renderer invariants, TypeScript checks, desktop and browser builds, plus deployed interaction smoke tests |
| **Kevin's focus** | Product evaluation and iteration, interface/accessibility refinement, desktop productization, public browser adaptation, technical documentation, and verification |
...
### 1. Preventing recursive desktop capture
...
The active shader now keeps **optical correction separate from magnification**. The focal region applies the correction kernel to the original sampling coordinates rather than quietly enlarging the image. It also:
...
Separate validation targets estimate screen-space error. Successful model initialization is not treated as evidence that the mapping is accurate.
...
Prescription conversion is implemented in [`prescription.ts`](src/renderer/lib/optics/prescription.ts). PSF and active correction-kernel generation live in [`psf.ts`](src/renderer/lib/optics/psf.ts).
...
The workflow includes viewing-distance calibration, physical screen calibration using a known-size reference, Snellen-style acuity testing, astigmatism clock and fan interfaces, contrast comparison, and a clearly labeled heuristic result.
...
`npm test` compiles and runs nine deterministic checks covering:
...
- guided acuity, astigmatism, and contrast stages remain heuristic and need stronger scoring and validation before supporting reliable prescription estimation
...
The strongest next work is measurement and validation, not adding more controls: benchmark frame time and latency on defined hardware, quantify gaze-calibration error, compare optical models against predefined visual criteria, and expand controlled testing.
...
| Demo effects that could overstate correction | Remove hidden focal magnification and brightness-only fallback behavior | Keeps visible evidence aligned with optical correction itself |
| Prototype controls that could overstate functionality | Expose only controls wired to active runtime behavior | Keeps the interface aligned with the implementation |
...
**Credibility is an engineering property.** Runtime limits, demos, tests, and documentation should agree about what the system actually does.
...
docs/media/              Screenshots and technical diagrams
...
| **Interface and accessibility engineering** | Completed a substantial application-wide refinement covering typography, spacing, responsive layout, navigation, accessibility semantics, help graphics, and Refract branding |
| **Desktop productization** | Added the Refract app and tray icon system, icon-generation tooling, and Windows packaging support |
| **Public engineering work** | Prepared the public repository, refined the browser adaptation, added technical diagrams and documentation, aligned visible controls with runtime behavior, and added automated numerical and production verification |
